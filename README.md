...
> 原项目 [hmdp-plus](https://github.com/Noy-17/hmdp-plus) 由 **阿星不是程序员** 开发，在经典”黑马点评”基础上补齐了令牌桶限流、RabbitMQ 可靠消息、Lua 原子扣减、分库分表、对账一致性闭环等高并发能力。
>
> **本仓库在原作基础上进行了微服务架构魔改**，将单体应用拆分为 5 个独立 Spring Boot 服务，为后续接入 Spring Cloud Alibaba（Nacos + Gateway + OpenFeign + Sentinel）做好准备。

## 魔改说明

| 维度 | 原 hmdp-plus | 魔改版 |
|------|-------------|--------|
| 架构 | 单体 (hmdp-core-service) | **5 个微服务** + 共享 API 模块 |
| 跨域调用 | 进程内调用 | Bridge 模式（阶段四）→ OpenFeign（阶段五） |
| 服务发现 | 无 | Nacos（阶段五） |
| 网关 | 无 | Spring Cloud Gateway（阶段五） |
| 事务 | 本地事务 | Seata 分布式事务（阶段七） |

### 5 个微服务

| 服务 | 端口 | 职责 |
|------|------|------|
| `hmdp-shop-service` | 8081 | 商铺查询、商铺类型、GEO 搜索 |
...
“黑马点评”是 Java 学习圈中广为人知的实战项目，围绕”本地生活与商户运营”场景展开：商户浏览与查询、优惠券发放与抢购、达人探店、好友关注、签到与 UV 统计等。它把”八股文”转化为可落地的工程方案。

但原版在面试中容易被深问：
- 流量突发时如何动态限流？
- Redis/MQ 宕机或数据丢失怎么办？
- 数据库与 Redis 的库存不一致怎么办？

**阿星不是程序员** 推出的 hmdp-plus 针对这些问题给出了完整的工程方案：Spring Boot 4 + 令牌桶限流 + RabbitMQ 可靠消息 + 分库分表 + 对账一致性闭环。

本仓库在此基础之上，进一步将单体架构拆分为微服务，按业务域独立部署，为 Spring Cloud Alibaba 全套体系铺路。
