...
>
> **本仓库是在原作基础上的魔改版本**，主要变更：(1) Spring Boot 3 → 4，(2) Kafka → RabbitMQ，(3) 单体架构拆分为 5 个独立微服务，(4) 为 Spring Cloud Alibaba 全套体系铺路（Nacos + Gateway + OpenFeign + Sentinel + Seata）。
...
| 服务 | 端口 | 职责 |
|------|------|------|
| `hmdp-shop-service` | 8081 | 商铺 CRUD、MQ 同步事件发布 |
| `hmdp-search-service` | 8086 | Elasticsearch 搜索引擎（名称/类型/GEO 搜索） |
| `hmdp-user-service` | 8082 | 用户注册/登录/信息管理 |
...
| ORM | MyBatis-Plus 3.5.7 + ShardingSphere 5.3.2 |
| 数据库 | MySQL 8.0 (双库分片: hmdp_0 / hmdp_1) |
| 缓存/锁/队列 | Redis 7, Redisson 3.52.0, Caffeine |
...
“黑马点评”是 Java 学习圈中广为人知的入门实战项目，围绕”本地生活与商户运营”场景展开：商户浏览与查询、优惠券发放与抢购、达人探店、好友关注、签到与 UV 统计等。

但原版在面试中容易被深问：流量突发时如何动态限流？Redis/MQ 宕机或数据丢失怎么办？数据库与 Redis 的库存不一致怎么办？

**阿星不是程序员（JavaUp）** 推出的 [hmdp-plus](https://gitee.com/java-up-up/hmdp-plus) 针对这些问题给出了完整的工程方案：Spring Boot 3 + Kafka + 令牌桶限流 + 分库分表 + 对账一致性闭环。本仓库在此基础上进一步魔改。
...
- **MQ 可靠性不足**
    ...
    - 🔹 **Plus 方案：使用 RabbitMQ，并在生产端/消费端确认、指数退避重试、DLQ/延迟队列、消费幂等、发送失败/消费失败/消费超时的各种处理。**
...
这会启动 MySQL（双库 `hmdp_0` / `hmdp_1`）、Redis、RabbitMQ、Nacos、Sentinel、Elasticsearch、Seata。

## 2. 配置连接信息

如果中间件不在本机，复制环境变量模板并修改：

```bash
cp .env.example .env
# 编辑 .env 中的 MYSQL_HOST / REDIS_HOST / RABBITMQ_HOST 等
```

同时修改各服务 `src/main/resources/shardingsphere.yaml` 中的 MySQL host / username / password。
...
# Gateway + 5 个微服务：
java -jar hmdp-gateway/target/hmdp-gateway-0.0.1-SNAPSHOT.jar               # 网关 → :8080
java -jar hmdp-shop-service/target/hmdp-shop-service-0.0.1-SNAPSHOT.jar     # 商铺 → :8081
java -jar hmdp-user-service/target/hmdp-user-service-0.0.1-SNAPSHOT.jar     # 用户 → :8082
java -jar hmdp-voucher-service/target/hmdp-voucher-service-0.0.1-SNAPSHOT.jar  # 秒杀 → :8083
java -jar hmdp-blog-service/target/hmdp-blog-service-0.0.1-SNAPSHOT.jar     # 博客 → :8084
java -jar hmdp-follow-service/target/hmdp-follow-service-0.0.1-SNAPSHOT.jar  # 关注 → :8085
java -jar hmdp-search-service/target/hmdp-search-service-0.0.1-SNAPSHOT.jar  # 搜索 → :8086
```

所有 API 请求通过 Gateway (`:8080`) 统一入口，Nacos 负责服务发现与负载均衡。
...
浏览器访问 `http://localhost:5173`，API 请求自动代理到后端服务。
...
# 六、各种中间件宕机和数据丢失的解决方案
将真实生产环境中遇到的宕机问题、数据丢失、问题排查等实际逻辑的解决方案，都会在项目中进行应用。
